...
const Layout: React.FC<LayoutProps> = ({ 
  ...
  const isAdminOrStaff = isOwner || isProfesor;

  const getAdminMenu = (): { id: AdminSection; label: string; icon: string }[] => {
    const menu: { id: AdminSection; label: string; icon: string }[] = [
      { id: 'dashboard', label: isOwner ? 'Panel General' : 'Dashboard', icon: isOwner ? '⚡' : '📊' },
    ];

    if (isOwner) {
       menu.push(
        { id: 'auditoria', label: 'Auditoría Financiera', icon: '💰' },
        { id: 'sedes', label: 'Gestión de Sedes', icon: '🏢' }
       );
    } else if (isProfesor) {
      menu.push(
        { id: 'materias', label: 'Mis Grupos', icon: '📋' },
      );
    }
    return menu;
  };
            ...
            `}>
              <div className="p-6 md:p-10 space-y-2">
                <div className={`mb-8 md:mb-10 px-6 py-6 rounded-[28px] md:rounded-[32px] border ${isOwner ? 'bg-black text-white border-black' : 'bg-zinc-50 border-zinc-200'}`}>
                  ...
                  <p className={`text-sm font-black italic truncate tracking-tighter uppercase ${isOwner ? 'text-white' : 'text-next-green'}`}>{isOwner ? 'ADMINISTRACIÓN' : 'DOCENTE'}</p>
                </div>
                {adminMenu.map((item) => (
                  <button
                    key={item.id}
